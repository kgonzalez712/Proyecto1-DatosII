...
func _fixed_process(delta):
	...
	if get_node("KinematicBody2D").get_global_pos().y < 0 or get_node("KinematicBody2D").get_global_pos().x > 800 or get_node("KinematicBody2D").get_global_pos().y > 720 or get_node("KinematicBody2D").get_global_pos().x < 0:
		get_node(".").queue_free()
